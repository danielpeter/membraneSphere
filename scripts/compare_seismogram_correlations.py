...
# approximate minimum period of simulation T_min = 40 s
TMIN = 40.0
###############################################################
...
def plot_correlations(out_dir,ref_dir):
    ...
    # seismogram files
    files = glob.glob(out_dir + '/seismo.*.dat')
    if len(files) == 0:
        print("no seismogram files 'seismo.*.dat' found")
...
def usage():
    ...
    print("  with")
    print("     directory1 - directory holding seismogram files (seismo.***.dat), e.g. OUTPUT/")
    print("     directory2 - directory holding corresponding reference seismogram files, e.g. REF_OUT/")
