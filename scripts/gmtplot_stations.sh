...
gawk '{if((substr($1,1,1)!="#")&&($1!=""))print($1,$2,0.05)}' $datafilename | \
	 psxy $verbose $region $projection -K -G002/170/002 -Sc $offsets $portrait -: > $ps_filename
...
#psscale -D9/-1/4/0.5h -Ba.1f1:"km/s": -C$colormap -V -O -K >> $ps_filename
...
echo
echo image plotted to $datafilename.pdf
echo
