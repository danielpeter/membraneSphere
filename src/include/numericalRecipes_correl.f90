...
!-----------------------------------------------------------------------
      ...
      COMPLEX(SPC), DIMENSION(:,:), INTENT(INOUT) :: data
      INTEGER(I4B), INTENT(IN) :: isign
! Replace seach row(constant first index) of data(1:M,1:N) by its discrete Fourier trans-
...
! When isign=1, calculates the Fourier transform of a set of N real-valued datapoints,
! input in the array data. If the optional argument z data is not present, the data are replaced
...
! respectively. If the complex array z datao f length N/2 is present, data is unchanged and
! the transform is returned in z data. N must be a power of 2. If isign = -1, this routine
...
! with z data absent, or in z data.
...
! returns: first derivated function value at xin, error estimation in err
      ...
      a(1,1)=(mydrsple(i1,i2,X,Y,Q,ilength,dble(xin+hh))-mydrsple(i1,i2,X,Y,Q,ilength,dble(xin-hh)))/(2.0_sp*hh)
      ...
      do i = 2,NTAB
        ...
        a(1,i)=(mydrsple(i1,i2,X,Y,Q,ilength,dble(xin+hh))-mydrsple(i1,i2,X,Y,Q,ilength,dble(xin-hh)))/(2.0_sp*hh)
...
!-----------------------------------------------------------------------
      ...
      splineRepresentation = mydrsple(i1,i2,X,Y,Q,ilength,location)
      return
...
!-----------------------------------------------------------------------
      ...
      do i = 1,size(trace)
        ...
        spline_func(i) = mydrsple(i1,i2,X,Y,Q,ilength,trace(i))
      enddo
