...
!Given an M ×N tabulated function ya, and N tabulated independent variables x2a, this 
!routine constructs one-dimensional natural cubic splines of the rows of ya and returns the 
!second derivatives in the M×N array y2a. (The array x1a is included in the argument 
...
!GivenarraysxandyoflengthNcontainingatabulatedfunction, i.e.,y 
!i =f(xi),withx1<x2<... <xN, andgivenvaluesyp1andypnfor theﬁrstderivativeof theinterpolating 
!functionat points 1andN, respectively, this routine returns anarray y2of lengthN 
!that contains thesecondderivatives of theinterpolatingfunctionat thetabulatedpoints 
! xi. If yp1and/or ypnareequal to 1×10^30 or larger, theroutineis signaledtoset the 
!correspondingboundaryconditionfor anatural spline, withzerosecondderivativeonthat 
!boundary. 
	USE nrtype; USE nrutil, ONLY : assert_eq
	USE nr, ONLY : tridag
	IMPLICIT NONE
...
!and y2a are all of the same size.
  ...
  ! Wewill ﬁndtheright placeinthetablebymeans of locate’sbisectionalgorithm. This is 
  ! optimal if sequential callstothisroutineareatrandomvaluesof x. If sequential callsarein 
  ! order, andcloselyspaced, onewoulddobetter tostorepreviousvaluesof kloandkhiand 
  ! test if theyremainappropriateonthenext call.  
	khi=klo+1
	h=xa(khi)-xa(klo)
