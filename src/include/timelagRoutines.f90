...
!-----------------------------------------------------------  
      ...
      if( ierror .ne. 0) call stopProgram('resample error')
...
! returns: subsample precision maximum
      ...
      if( indexmax .gt. 1 .and. indexmax .le. length/2) then
      ...
      if( indexmax .ge. length/2+1 .and. indexmax .lt. length)then
        pointleft=crosscorrelation(indexmax-1)
        pointright=crosscorrelation(indexmax+1)
      endif      
...
! returns: seismo array and dt
      ...
      do i=1, FILELINES
        if(i .ge. ZEROLINE .and. index .lt. size) then
          ...
          if( time - start .ge. 0.0) then ! starts with times from 'start time' on
            index=index+1
            seismo(1,index) = time
            seismo(2,index) = displace
