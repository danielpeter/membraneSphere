 ...
 1002 format(4(f11.4,1x),1x,f11.6,1x,f11.7)
    ...
    do while( ierror .eq. 0 )
      ...
      if( ierror .eq. 0 ) then
        ...
        ! some statistics      
        if( datum .gt. datummax ) datummax = datum
        if( datum .lt. datummin ) datummin = datum
        ...
        if(epidelta.lt.epicentraldistance_min) then
          epicentraldistance_min = epidelta
          epiminIndex = datacount
        endif
        if(epidelta.gt.epicentraldistance_max) then
  ...
  ! handle events
    ...
    do j=1,datacount
      ...
      if( doCalc(j) ) then
        ...
        ! benchmark
        if( MASTER .and. mod(j,500) .lt. nprocesses ) then
        ...
        if( j .gt. dataLoopIndex) dataLoopIndex = j
      endif
    enddo
  ...
  ! and calculations should be done
    ...
    if( .not. PARALLELSEISMO ) then
      ...
      if( mod((index-rank+2*nprocesses-1),nprocesses) .eq. 0 ) then 
        doCalc=.true.
      else
        doCalc=.false.
      endif
  ...
  ! sync with master process and print to file
    ...
    if( MASTER ) then
      ...
      if( restprocesses .ge. nprocesses ) then
